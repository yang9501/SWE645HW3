    ...
    stage("Deploying on cluster through Rancher"){
      steps{
        script {
          ...
          sh 'kubectl --kubeconfig /var/lib/jenkins/.kube/config set image deployment/frontend container-0=yang9501/frontend:latest'
          sh 'kubectl --kubeconfig /var/lib/jenkins/.kube/config set image deployment/surveysystem container-0=yang9501/surveysystem:latest'
